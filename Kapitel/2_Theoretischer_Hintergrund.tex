...
untereinander (\acfirst{B2B}) \cite{turban_ecommerce}. Der Begriff schließt
...
der \acs{PCI-DSS}-Scope des Betreibers erheblich reduziert wird
\cite{stripe_security_guide}.
...
Tailwind CSS ist ein utility-first \acs{CSS}-Framework, das
Entwicklern vorgefertigte \acs{CSS}-Hilfsklassen bereitstellt,
die direkt im \acs{HTML}-Markup verwendet werden. Im Gegensatz
zu klassischen \acs{CSS}-Frameworks wie Bootstrap definiert
...
\acs{CSS} geschrieben werden muss und Stile direkt im Markup
...
der Web Content Accessibility Guidelines (\acfirst{WCAG}~2.1), ist
dabei ein wesentliches Qualitätsmerkmal moderner
Webanwendungen. \acs{WCAG}~2.1 legt unter anderem
...
\textit{Bazario} über \acs{REST}-basierte \acfirst{HTTP}-Schnittstellen.
...
speichern muss. Auf der Clientseite werden diese \acs{HTTP}-Anfragen mithilfe von Axios
umgesetzt, einem verbreiteten, Promise-basierten \acs{HTTP}-Client für
...
Für die Absicherung der \acs{API}-Endpunkte setzt
...
weiteren \acs{API}-Anfrage im Authorization-Header übermittelt
...
Neben der \acs{REST}-basierten Kommunikation setzt
