...
Unternehmen und Konsumenten (\acs{B2C}\acused{B2C}) sowie zwischen Unternehmen
untereinander (\acs{B2B}\acused{B2B})
\cite{turban_ecommerce}. Der Begriff schließt
...
eine Bibliothek vorgefertigter, \ac{PCI-DSS}-konformer UI-Komponenten
...
Architekturansätze unterteilen: Multi-Page Applications (\acs{MPA})
und Single-Page Applications (\acs{SPA}). Bei MPAs wird bei jeder
...
Warenkorbinhalt. Diese Zustände werden dabei nicht in einem
...
Daten, die über eine \ac{API} vom Backend abgerufen werden. Diese
...
Ansichten der Anwendung anhand der \ac{URL} darzustellen. React
...
rollenbasierten Zugriffskontrolle (\ac{RBAC}), das den Zugriff
...
der Anwendung. Angular verfolgt einen stark vorgebenden Ansatz
...
der Web Content Accessibility Guidelines (\ac{WCAG}~2.1), ist
...
\textit{Bazario} über REST-basierte \ac{HTTP}-Schnittstellen.
\ac{REST} ist ein Architekturstil für Webschnittstellen, der auf 
