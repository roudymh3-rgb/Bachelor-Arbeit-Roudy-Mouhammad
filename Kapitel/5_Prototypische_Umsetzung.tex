...
dessen Inhalt im Cache gehalten wird. Jede Änderung, etwa das
Hinzufügen eines Elements, wird dabei im Cache abgelegt. Da der
Cache jedoch nur besteht, solange die Seite geöffnet ist, muss
...
bereits bestehenden UI-Store aufgenommen, sondern in einem
eigenständigen Store zusammengefasst. Diese Trennung folgt dem
bereits in Abschnitt~\ref{subsec:state-management} eingeführten
Prinzip der Separation of Concerns: Der Auth-Store ist
ausschließlich für die Authentifizierung zuständig, der UI-Store
...
abgerufen und in derselben Kartenansicht dargestellt, die auch
auf der öffentlichen Katalogseite verwendet wird (vgl.
Abschnitt~\ref{subsec:impl_produktlisting}). Das Löschen eines
