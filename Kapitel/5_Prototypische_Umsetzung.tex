...
ist als eigenständige, wiederverwendbare Komponente umgesetzt, die
auf der Dienstleistungs-Detailseite eingebunden wird. Sie
durchläuft drei Zustände.
 
Im Ausgangszustand zeigt sie einen Monatskalender und die Zeitzone
des Anbieters. Jeder Tag lässt sich auswählen; welche Zeitfenster er
enthält, ergibt sich erst aus der Antwort des Backends. Fällt ein Tag
in einen Abwesenheitszeitraum oder sieht das Wochenschema für ihn
keine Arbeitszeit vor, erscheint an Stelle der Liste ein Hinweis, dass
für dieses Datum keine Zeitfenster verfügbar sind. Ein nicht
verfügbarer Tag wird damit nicht ausgeblendet, sondern benannt --
dieselbe Entscheidung wie bei den Fristen für Stornierung und
Verlegung (vgl.
Abschnitt~\ref{subsec:verkaeufer_dashboard_konzept}). Die
Schaltfläche zur Übernahme in den Warenkorb ist zunächst deaktiviert,
da noch kein Termin feststeht.
...
Zusammenfassung der ausgewählten Buchung, und die Schaltfläche wird
aktiv.
 
\begin{figure}[H]
\centering
\includegraphics[width=0.42\textwidth]{Bilder/screenshot_buchung_auswahl.png}
\caption{Terminauswahl nach Wahl von Tag und Zeitfenster}
\label{fig:buchung_auswahl}
\end{figure}
 
Dass die Schaltfläche bis zur vollständigen Auswahl deaktiviert
bleibt, ist bewusst gewählt: Eine unvollständige Buchung kann nicht
in den Warenkorb übernommen werden, und der Nutzer erhält diese
Rückmeldung, bevor er die Aktion auslöst, statt danach als
Fehlermeldung (vgl.
...
der Leistungserbringung enthält. Diese Angaben werden später mit dem
Auftrag an das Backend übertragen (vgl.
Abschnitt~\ref{subsec:impl_stripe}).
...
Die Zusammenfassung des Warenkorbs weist Produkte und
Dienstleistungen getrennt aus, obwohl beide dasselbe Modell
verwenden. Damit bleibt erkennbar, wie viele terminierte Positionen
der Vorgang enthält -- eine Angabe, die für den weiteren Verlauf
erheblich ist, da nur diese Positionen später storniert oder verlegt
werden können.
...
Abschnitt~\ref{subsec:impl_dienstleistungsverwaltung}).
...
Solange eine Frist läuft, nennt der zugehörige Block den Zeitpunkt
ihres Endes, und die Schaltfläche ist auslösbar. Der Nutzer erfährt
damit vor der Handlung, bis wann sie möglich ist.
...
verstrichen ist. Beide Schaltflächen bleiben dabei sichtbar. Die
Alternative, sie zu entfernen, wäre technisch einfacher gewesen,
hätte dem Nutzer aber nicht mitgeteilt, warum eine erwartete Funktion
fehlt. Dieselbe Entscheidung liegt der Terminauswahl zugrunde, in der
ein nicht verfügbarer Tag ebenfalls benannt statt ausgeblendet wird
(vgl. Abschnitt~\ref{subsec:impl_terminauswahl}).
 
Die Umbuchung führt auf eine eigene Seite, die dieselbe
Terminauswahl verwendet wie die Erstbuchung. Die Komponente wird
damit an zwei Stellen der Anwendung eingesetzt, ohne dass die Logik
zur Ermittlung verfügbarer Zeitfenster ein zweites Mal umgesetzt
werden musste. Neben der Auswahl zeigt die Seite den bisherigen
Termin, sodass der Wechsel nachvollziehbar bleibt.
 
Eine Stornierung löst dagegen keine erneute Auswahl aus, sondern eine
Rückerstattung über das Backend. Deren Stand wird anschließend an der
Buchung selbst geführt, sodass der Nutzer den Vorgang bis zum
Abschluss verfolgen kann.
