...
vollständig im Backend, und die Zahlungsseite selbst wird von diesem
bereitgestellt (vgl. Abschnitt~\ref{sec:technologieauswahl}).

Ausgelöst wird der Vorgang über die Schaltfläche in der
Warenkorbübersicht. Ist der Nutzer nicht angemeldet, öffnet die
Funktion über den \acs{UI}-Store den Anmeldedialog und bricht ab.
Andernfalls läuft die Checkout-Operation an, die als Mutation
umgesetzt ist, da sie den Datenbestand auf dem Server verändert (vgl.
Abschnitt~\ref{subsec:impl_tanstackquery}). Sie durchläuft drei
Schritte: Zunächst wird über die Feature-\acs{API} ein Auftrag
angelegt, dessen ID das Backend zurückgibt. Anschließend wird
für jede Position eine weitere Anfrage gestellt, die sie diesem
Auftrag zuordnet. Zuletzt wird die Zahlungssitzung angefordert; das
Backend liefert die Adresse der Zahlungsseite zurück, auf die das
Frontend den Browser weiterleitet.
...
Positionstyp unterscheiden: Für ein Produkt genügen ID und
...
Die Erfolgsseite ruft die Auftragsdaten ab und zeigt Auftragsnummer,
Status und Betrag an; darunter stehen die beiden Wege in die
Übersichten für Bestellungen und Buchungen.
...
des Checkouts, da die Zahlung bis zuletzt scheitern kann.
...
Warenkorb noch der Datenbestand auf dem Server verändert wurde. Beide
...
trägt keine Schaltfläche mehr. Beide Übergänge sind als Mutationen
umgesetzt, die den zwischengespeicherten Bestand im Erfolgsfall
invalidieren (vgl. Abschnitt~\ref{subsec:impl_tanstackquery}), sodass
der neue Zustand unmittelbar erscheint.

\subsection{Bestellungen und Buchungen im Kontobereich}
...
eines Eintrags führt auf die Detailseite, die den Auftrag in eine
Zusammenfassung mit den Kopfdaten und die Liste der enthaltenen
Positionen gliedert.

Da ein Auftrag bereits vor der Zahlung entsteht (vgl.
Abschnitt~\ref{subsec:warenkorb_konzept}), unterscheidet die
Detailseite zwei Fälle. Bleibt ein Auftrag unbezahlt, tritt an die
Stelle der Positionsliste ein Bereich, der den Zustand benennt und
die beiden in Abschnitt~\ref{subsec:verkaeufer_dashboard_konzept}
vorgesehenen Wege anbietet. Bei einem bezahlten Auftrag erscheint
dagegen die vollständige Positionsliste, in der jede Position ihren
eigenen Zustand trägt, da Produkte und Dienstleistungen unabhängig
...
\begin{minipage}{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{Bilder/screenshot_bestelldetail.png}
\end{minipage}
\caption{Detailansicht eines Auftrags mit nicht abgeschlossener
  Zahlung (links) und eines bezahlten Auftrags mit Produkt- und
  Dienstleistungspositionen (rechts)}
\label{fig:auftrag_vergleich}
\end{table}

Dienstleistungspositionen führen die zugehörige Buchung mit Termin,
Zeitzone und Ort der Leistungserbringung mit. Dieselben Buchungen
sind zusätzlich über eine eigene Übersicht erreichbar. Die doppelte
Erreichbarkeit ist beabsichtigt: Wer von einer Bestellung ausgeht,
soll die zugehörige Buchung nicht in einer anderen Ansicht suchen
müssen.
...
Die Umbuchung führt auf eine eigene Seite, die dieselbe Terminauswahl
verwendet wie die Erstbuchung; die Komponente ist damit an zwei
Stellen eingesetzt, ohne dass die Ermittlung verfügbarer Zeitfenster
ein zweites Mal umgesetzt werden musste. Neben der Auswahl zeigt die
Seite den bisherigen Termin, sodass der Wechsel nachvollziehbar
bleibt. Eine Stornierung löst dagegen keine erneute Auswahl aus,
sondern eine Rückerstattung über das Backend, deren Stand
anschließend an der Buchung geführt wird.
