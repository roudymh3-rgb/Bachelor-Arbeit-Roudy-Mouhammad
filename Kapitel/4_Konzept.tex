...
Abschnitt~\ref{subsec:komponentenbasiert} vorgestellte Prinzip der Trennung von Darstellung und Datenbeschaffung wider:
Komponenten im Verzeichnis \texttt{components/} eines
Funktionsbereichs übernehmen ausschließlich die Darstellung, während
Datenzugriff und Geschäftslogik in den zugehörigen Ordnern
\texttt{api/} und \texttt{hooks/} gekapselt sind.
 
Das Projekt wird mit Vite als Build-Tool initialisiert, das eine
schnelle Entwicklungsumgebung und optimierte Produktions-Builds für
React-Anwendungen bereitstellt \cite{vite_docs}. Die Wurzel des
Projekts enthält Konfigurationsdateien wie \texttt{vite.config.ts},
...
Verzeichnis \texttt{stores/} sind die globalen Zustand-Stores
gebündelt (vgl.
...
Funktionsbereiche \texttt{cart/} für den Warenkorb,
\texttt{orders/} für die Bestellabwicklung, \texttt{connect/} für das
...
Netzwerke und Schnellzugriffe auf. Diese Inhalte sind entweder bereits
in der Kopfzeile erreichbar oder spielen für den Funktionsumfang der
Plattform keine Rolle. Verzichtet wird damit nicht auf
...
leiten sich daraus über feste Faktoren ab und ergeben die Werte sechs,
acht, zehn, vierzehn, achtzehn, zweiundzwanzig und sechsundzwanzig
Pixel. Dass überhaupt mehrere Stufen nötig sind, liegt an der
...
und der zugehörige Hinweis, trennt sie die Farbe. Diese Staffelung
ist auf den Konto- und Bestellseiten maßgeblich, wo zahlreiche
...
sich ein Kontrastverhältnis von 16,5:1, für gedämpften Text 5,3:1 auf
der Karte und 4,7:1 auf dem Seitenhintergrund; sämtliche Werte
erfüllen Erfolgskriterium 1.4.3 der Stufe AA. Für Bedienelemente und
Zustandsanzeigen gilt mit Kriterium 1.4.11 ein eigener Schwellenwert
von 3:1; der Fokusring erreicht 3,6:1 gegenüber einer Karte und
3,2:1 gegenüber dem Seitenhintergrund. Die Prüfung der übrigen
Bedienelemente erfolgt in Kapitel~\ref{chap:testen}.
...
und Anbieter (vgl. Abschnitt~\ref{subsec:katalog_konzept}). Rechts
stehen die Werkzeuge, die den Nutzer selbst betreffen: die Sprachwahl
(vgl. Abschnitt~\ref{subsec:mehrsprachigkeit_konzept}), der Zugang zu
den Nachrichten (vgl. Abschnitt~\ref{subsec:messaging_konzept}), der
...
werden gezielt dem betroffenen Feld zugeordnet, und der
Absende-Button wird während der Bearbeitung deaktiviert.
...
Nutzerdaten und Rollen. Diese wird im localStorage gespeichert, um den
Anmeldestatus über Neuladen und erneute Besuche hinweg zu erhalten,
und reaktiv über Zustand bereitgestellt (vgl.
Abschnitt~\ref{subsec:state-management-konzept}). Der Zugriff auf
geschützte Bereiche wird über ein dreistufiges Konzept sichergestellt:
Zunächst wird geprüft, ob der Anmeldestatus bereits bestimmt ist
(Ladezustand), dann ob der Nutzer angemeldet ist (sonst
...
Das Verfahren muss diese Behauptung deshalb prüfen, bevor es ein neues
Passwort entgegennimmt, und es tut das über das Postfach selbst: An
die angegebene Adresse geht ein Einmalkennwort, das nur dort ankommt.
Wer es zurücksenden kann, hat Zugriff auf dieses Postfach. Das
Kennwort gilt einmal und nur für eine begrenzte Zeit, sodass ein
liegen gebliebener Wert nicht dauerhaft verwendbar bleibt.
...
Seite, auf der ein neues Passwort gesetzt wird, ist die wirksamste des
gesamten Zugangs, denn wer sie erreicht, kann ein Konto übernehmen.
Sie darf deshalb nicht allein davon abhängen, welche Adresse in der
Anfrage steht, da diese Angabe aus dem Browser kommt und dort
veränderbar ist. Das Backend gibt aus diesem Grund nach der
erfolgreichen Prüfung des Kennworts einen Nachweis zurück, der an
genau diese Adresse gebunden ist, und verlangt ihn im letzten Schritt.
Erzeugung und Prüfung dieses Nachweises liegen vollständig im Backend
und sind in der begleitenden Arbeit zum Backend beschrieben. Für das
Frontend folgt daraus allein die Anforderung, die drei Schritte in der
vorgesehenen Reihenfolge zu führen und den Nachweis zwischen ihnen zu
übergeben.
...
einrichten und verknüpfen muss. Auf dieser Grundlage lässt sich der Zahlungsbetrag automatisiert
aufteilen: Ein Anteil verbleibt als Provision bei der Plattform, der
übrige Betrag wird dem Anbieter gutgeschrieben (vgl.
Abschnitt~\ref{subsec:marktplatztypen}). Die Provision ist als
prozentualer Anteil am Transaktionswert festgelegt und beträgt
gegenwärtig zehn Prozent. Sie wird, ebenso wie die Preise der bezahlten Platzierungen, über das
Admin-Dashboard des Backends gepflegt (vgl.
Abschnitt~\ref{subsec:werbung_konzept}) und ist damit ein
konfigurierbarer Wert und keine feste Größe der Anwendung. Das
Frontend berechnet den Anteil nicht, sondern führt je Auftragsposition
den vom Backend gelieferten Nettobetrag mit; dieser erscheint in der
Übersicht der Verkäufe auf der Kontoseite.
...
Zahlungsstatus und beim Zahlungsdienst der Verbindungszustand. Daneben
wird genau die Aktion angezeigt, die in diesem Zustand möglich ist, etwa das
...
Bedeutung trägt, gilt hier also mit einer Einschränkung: Sie
beschleunigt das Erfassen, trägt die Angabe aber nicht allein; eine
Unterscheidung, die für Nutzer mit eingeschränktem Farbsehen den
Ausschlag gibt.
...
Buchung damit nicht aus dem Zusammenhang erschließen, sondern findet an jedem Eintrag den nächsten möglichen Schritt. Sichtbarkeit des Systemstatus und Wiedererkennung statt Erinnerung greifen hier ineinander (vgl. Abschnitt~\ref{subsec:gestaltungsprinzipien}).
...
zugrunde. Ein einheitlicher Preis je Anzeige wäre die einfachere Regel
gewesen, hätte aber die Dauer unberücksichtigt gelassen: Eine einmal
bezahlte Platzierung bliebe dann unbegrenzt stehen und verdrängte
nachfolgende. Die Obergrenze von sieben Tagen begrenzt zusätzlich, wie
lange ein einzelner Anbieter eine Fläche halten kann.
...
zur Geschäftslogik der Plattform und lassen sich ändern, ohne dass die
Oberfläche davon berührt wäre.
...
entbehrlich wäre: Der Administrator kann eine Platzierung nur dann auf
ihre Zulässigkeit prüfen, wenn ihm auch das Bildmaterial vorliegt. Der
Anbieter sieht in seinem Kontobereich deshalb zwei Angaben
nebeneinander: den Zustand der Platzierung, also Zahlung ausstehend,
in Prüfung, freigegeben oder abgelehnt, und den Zustand der Zahlung.
Beide werden getrennt geführt, weil sie sich unabhängig voneinander
ändern. Eine Platzierung kann bezahlt und dennoch abgelehnt sein.
...
erstattet; der Zustand der Zahlung wechselt dann von
\textit{Bezahlt} auf \textit{Erstattet}. Zieht der Anbieter seine
Platzierung dagegen nach der Zahlung selbst zurück, erfolgt keine
Erstattung, denn die Prüfung hat dann bereits stattgefunden und ist
die erbrachte Leistung. Die Unterscheidung folgt daraus, wer die
Ursache setzt: Im ersten Fall verweigert die Plattform die
vereinbarte Leistung und hat keinen Anspruch auf das Entgelt; im
zweiten hat der Anbieter sie in Anspruch genommen und danach auf ihr
Ergebnis verzichtet. Dass die Zahlungsangabe dabei mitwechselt, ist
kein Randdetail. Bliebe sie auf \textit{Bezahlt} stehen, führte die
Ansicht zwei Angaben nebeneinander, von denen eine nicht mehr gilt,
und der Anbieter müsste aus der Ablehnung selbst schließen, was mit
seinem Geld geschehen ist.
 
Jede Platzierung trägt schließlich ein Ablaufdatum. Ihre
Durchsetzung, also das regelmäßige Prüfen der Daten und das Entfernen
abgelaufener Einträge, ist im Rahmen dieser Arbeit nicht umgesetzt
worden und in Kapitel~\ref{chap:fazit} als offener Punkt
aufgeführt. Die Preisbildung nach Tagen setzt sie voraus; ohne sie
bleibt die Dauer eine Angabe ohne Wirkung.
...
eines externen Zahlungsdienstes geführt hat,wird auch hier ein externer Dienst eingebunden, statt die
Fähigkeit selbst aufzubauen; welcher, ist in
Abschnitt~\ref{sec:technologieauswahl} festgelegt.
...
beiden Beteiligten erreichen. Zweitens muss der Zugang zu einem solchen Kanal geprüft werden. Ob ein Nutzer Teilnehmer einer Unterhaltung ist, kann nur das Backend beantworten, da nur dort die Unterhaltung gespeichert ist. Der Kanal ist damit nicht
...
wird. Wer zu diesem Zeitpunkt nicht verbunden ist, erhält die
...
gesendet, zugestellt und gelesen. Der Absender erkennt daran, ob der Empfänger sie gelesen hat, ein Fall der Sichtbarkeit des
Systemstatus (vgl. Abschnitt~\ref{subsec:gestaltungsprinzipien}).