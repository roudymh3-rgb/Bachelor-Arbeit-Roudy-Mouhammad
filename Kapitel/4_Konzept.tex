...
% TODO: Anzeige verfügbarer Zeitfenster auf der Dienstleistungs-
% Detailseite (basierend auf Arbeitszeiten und Abwesenheiten des
% Anbieters), Auswahl eines Termins, Übernahme der Buchung in den
% Warenkorb (vgl. 4.3.2 und 4.3.3).
...
einem gemeinsamen Modell ab, das für beide Produkte und Dienstleistungsbuchungen eine
...
hinzugefügte Artikelarten auch nach einem Neuladen der Seite
...
enthaltenen Artikelarten, getrennt nach Produkten und
Dienstleistungen, sowie eine Zusammenfassung mit Anzahl je
Artikelarten und Zwischensumme.
...
\textit{requires payment} an, ergänzt ihn um die Artikelarten des
...
zusammengestellten Artikelarten führt.
...
Zahlung auf die Seite des Anbieters wechselt, verliert die Plattform
die unmittelbare Sicht auf den Vorgang. Sie erfährt weder, ob gezahlt
wurde, noch von wem. Genau deshalb wird der Auftrag vorab im Backend
angelegt und der Zahlungsstatus im Anschluss mit dem Anbieter
abgeglichen.
...
\subsection{Anbieter- und Verkäufer-Dashboard}
...
Werkzeuge.
Die rechte Spalte ist nach Rollen gegliedert, nicht nach Funktionen.
Jede zugewiesene Rolle erhält einen eigenen Arbeitsbereich; ein
Dienstleister sieht folglich einen Anbieter- und einen Kundenbereich
untereinander, da er nach dem Rollenmodell aus
Abschnitt~\ref{sec:rollenmodell} zugleich Kunde ist. Der
Anbieterbereich führt zur Verwaltung von Dienstleistungen und
Verfügbarkeiten, zu den eingegangenen Buchungen sowie zur Anbindung an
den Zahlungsdienst und zur Einnahmenübersicht (vgl.
Abschnitt~\ref{subsec:warenkorb_konzept}), der Kundenbereich zur
eigenen Bestellhistorie und zur Verfolgung eigener Buchungen; darunter
folgen rollenunabhängige Funktionen wie Passwortänderung,
Rollenwechselantrag und Kontolöschung. Nicht zugewiesene Rollen
erzeugen keinen Arbeitsbereich, sodass die Oberfläche ausschließlich
die tatsächlich verfügbaren Funktionen zeigt. Damit wird die in
Abschnitt~\ref{subsec:rbac} beschriebene rollenbasierte
Zugriffskontrolle auf der Ebene der Benutzerführung umgesetzt.
...
andernfall würden in einer Liste Vorgänge nebeneinander angezeigt werden, bei denen
...
Zu jedem Eintrag wird der aktuelle Zustand angezeigt, bei Buchungen
der Bearbeitungsstand, bei Bestellungen der Zahlungsstatus und beim
Zahlungsdienst der Verbindungszustand, und daneben genau die Aktion,
die in diesem Zustand möglich ist, etwa das Bestätigen einer
angefragten Buchung oder das Nachholen einer offenen Zahlung. Der
Systemzustand ist damit erkennbar, ohne dass eine Detailansicht
...
% TODO (Roudy): Die Begruendung "da ein Produkt nicht
% terminiert gebucht, sondern direkt in den Warenkorb gelegt
% wird" ist meine eigene Erklaerung, nicht aus dem Meeting oder
% von dir bestaetigt. Bei Bedarf pruefen/umformulieren oder mit
% Mentor absichern.

% TODO: Buchungskalender, Bestellübersicht.
