...
zur \acs{PCI-DSS}-konformen Zahlungserfassung gewährleistet.
...
Benutzeroberflächen effizient unterstützt und gegenüber
Angular und Vue.js die in Abschnitt~\ref{subsec:react}
beschriebenen Vorteile hinsichtlich Flexibilität und
Ökosystem bietet \cite{comparative_spa_frameworks}.

Zustand wurde für das globale State Management gewählt.
Die Bibliothek basiert auf einem hook-basierten Ansatz
und lässt sich ohne zusätzliche Provider-Wrapper nahtlos
in die React-Komponentenstruktur integrieren (vgl.
Abschnitt~\ref{subsec:state-management}).
Die Session einschließlich der Nutzerrollen sowie der
Warenkorbinhalt werden in eigenständigen
...
Axios wurde als \acs{HTTP}-Client für die Kommunikation mit dem
Backend gewählt, ohne dass dabei ein einzelner, entscheidender
Grund ausschlaggebend war. Die Bibliothek stellt die
grundlegende Infrastruktur für die \acs{HTTP}-Kommunikation bereit
...
React Hook Form und Zod wurden für die Formularverarbeitung
und die clientseitige Validierung gewählt (vgl.
Abschnitt~\ref{subsec:formulare-validierung}). Formulareingaben
werden vor dem Absenden gegen Zod-Schemata geprüft, die bewusst
mit den Validierungsregeln des Backends übereinstimmen;
serverseitig gemeldete Fehler werden dem betroffenen
Eingabefeld zugeordnet
\cite{react_hook_form_docs, zod_docs}.

Tailwind CSS und shadcn/ui wurden als \acs{UI}-Bibliotheken
gewählt. Tailwind CSS ermöglicht eine schnelle und
konsistente Gestaltung der Benutzeroberfläche durch
vorgefertigte Hilfsklassen, ohne dass separates \acs{CSS}
...
durch eine Sammlung vorgefertigter, barrierefreier und
vollständig anpassbarer React-Komponenten (vgl.
Abschnitt~\ref{subsec:ui-bibliotheken}) \cite{shadcn_docs}.

Vite wird als Build-Tool eingesetzt, da es eine schnelle
Entwicklungsumgebung und optimierte Produktions-Builds für
React-Anwendungen bereitstellt (vgl.
Abschnitt~\ref{subsec:komponentenarchitektur})
\cite{vite_docs}. Für die Echtzeitkommunikation des
Messaging-Systems wird die Bibliothek \texttt{pusher-js}
verwendet, da das Backend Pusher als Broadcast-Dienst
einsetzt; das Frontend abonniert darüber die relevanten
Channels (vgl. Abschnitt~\ref{subsec:rest-api})
\cite{pusher_docs}.
...
\acs{PCI-DSS}-konforme Frontend-Integration bereitstellt, die
...
aufbaut. Aus Frontend-Sicht bestimmt Sanctum die Handhabung der Bearer Tokens, die nach erfolgreichem Login gespeichert und bei jeder \acs{API}-Anfrage im Authorization-Header mitgesendet werden  \cite{laravel_sanctum_docs}. Abgelaufene oder ungültige Tokens werden vom Frontend erkannt und die zugehörige Sitzung automatisch verworfen.
...
\textbf{Begründung} \\
\hline
React & \acs{UI}-Framework &
Komponentenbasiert, großes Ökosystem, weit verbreitet \\
\hline
Zustand-Bibliothek & State Management &
...
\hline
shadcn/ui & \acs{UI}-Komponenten &
Barrierefrei, vollständig anpassbar,
kein Paket-Lock-in \\
\hline
Stripe Elements & Zahlungsabwicklung &
\acs{PCI-DSS}-konform, nahtlose React-Integration \\
\hline
Laravel Sanctum & Authentifizierung &
Tokenbasiert, Bearer Tokens, rollenbasierte
Zugriffskontrolle auf \acs{API}-Endpunkte \\
\hline
React Hook Form & Formularverwaltung &
Hook-basiert, geringe Neu-Renderings,
koppelbar mit Schema-Validierung \\
\hline
Zod & Validierung &
...
\hline
pusher-js & Echtzeitkommunikation &
WebSocket-Anbindung über Pusher,
vom Backend vorgegeben \\
\hline
